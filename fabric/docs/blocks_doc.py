...
SECTIONS = [
    ...
    ("Memory units", [
        ...
        "The memory unit's adapter turns the program's memory commands into these: op 0 and 1 are the mover's reads and "
        "writes between beat addresses and the buffer, op 2 the append of the token in the buffer, op 3 the scan of the "
        "context's index into the top-K ids, op 4 the rows command that turns a selection into requests (the window in "
        "page runs, then one record per chosen block) and lays the reader's rows into the head's buffer.",
    ]),
]
