...
| psram_clk | 16 | N row inside the memory rows |
| VDD_CORE | 177 | interior checkerboard |
| GND | 223 | interior checkerboard and lane returns |
| VDD_IO_1V8 | 24 | interior |
| VDD_PLL_0V9 | 2 | interior |

Signal balls deeper than the outer 5 rows: 16 (these need a microvia or build-up escape on the PCB).

## What the packaging house gets

* this map as `asic_ballmap.csv`, with the edge each interface must face;
* the die-edge assignment it implies: link ports on the S and S die edges, the memory PHYs on the N and E and W edges, management on the south, psram_clk on the N;
