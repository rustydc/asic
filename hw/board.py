...
class Board:
    ...
    def power_budget_w(self) -> tuple[float, float]:
        """Return (load watts, input watts after regulator efficiency)."""
        load = sum(self.classes[self.class_of(ref)]["tdp_w"] for ref in self.components)
    ...
    def summary_markdown(self) -> str:
        ...
        lines = [f"# {self.data['board']['name']} board summary", "",
                 ...
                 "## Bill of materials (major parts)", "", "| Class | Qty | Description | TDP each (W) |",
                 "| --- | ---: | --- | ---: |"]
        for part, spec in self.classes.items():
            qty = len(self.instances(part))
            if qty:
                lines.append(f"| {part} | {qty} | {spec['description']} | {spec['tdp_w']} |")
        lines += ["", "## Power", "", f"Load {load:.0f} W, input {input_w:.0f} W at "
